...
Model.purchase = function (date, address, cardNumber, cardHolder) {
  ...
  var order = {
    ...
    subtotal: this.user.shoppingCart.subtotal,
    tax: this.user.shoppingCart.tax,
    total: this.user.shoppingCart.total,
    items: [],
  };
  for (var item of this.user.shoppingCart.items) {
    var book = {
  ...
  this.user.shoppingCart = { items: [], qty: 0, total: 0, subtotal: 0, tax: 0 };
  this.orderShown = id_order;
  return order;
};
